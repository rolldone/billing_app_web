...
export class LoginClass extends BaseStateClass<StateType, PropType> {
    ...
    async submit() {
        try {
            let form_data = this.state.form_data
            let resData = await AuthService.login(form_data)
            let _return = resData.return;
            let _setting = resData.setting as SettingType;
            window.localStorage.setItem("token", _return)
            if (_setting.last_installer_at == null || _setting.last_installer_at == "") {
                window.location.replace("/user/setup")
                return;
            }
            window.location.replace("/user")
        } catch (error) {
            console.error("LoginClass - error :: ", error)
        }
    }
}
                    ...
                    <div>
                        <div className="mb-3">
                            ...
                            <input
                                type="email"
                                name="email"
                                value={methods.state.form_data.email}
                                onChange={methods.handleFormData.bind(methods)}
